...
@section('title', 'Actualizar perfil de atención')

@section('content_header')
    <h1>Actualizar perfil de atención {{ $attentionProfile->name }}</h1>
@stop

@section('content')

    <form action="{{ route('attention-profiles.update', $attentionProfile->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
            ...
            <div class="col-md-3">
                <x-adminlte-card title="Crear servicio" theme="primary" icon="fas fa-edit">
                    <x-adminlte-input name="name" label="Nombre" value="{{ old('name', $attentionProfile->name) }}" />
